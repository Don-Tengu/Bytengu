...
test("device login asks xAI with the bytengu referrer", async () => {
  ...
  const issued = await requestDeviceCode(fetchImpl);
...
test("bearer uses a fresh oauth token, refreshes an expiring one, and otherwise asks for login", async () => {
  ...
  try {
    ...
    assert.equal(await currentBearer({ env: {}, path, now: () => 1_000 }), "fresh-access");
    ...
    assert.equal(await currentBearer({ env: {}, path, fetch: fetchImpl, now: () => 1_000 }), "new-access");
    const saved = await readAuthFile(path);
    assert.equal(saved.default, "xai");
    assert.equal(saved.providers.xai?.type === "oauth" ? saved.providers.xai.refresh : "", "new-refresh");

    await writeAuthFile(path, { providers: {} });
    assert.equal(await currentBearer({ env: { XAI_API_KEY: "env-key" }, path }), "env-key");
    await assert.rejects(() => currentBearer({ env: {}, path }), /bun run login/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
